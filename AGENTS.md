...
La CLI no toca SQLite: llama por HTTP a la instancia. Exige `--url` y `--token`
en cada invocación; no lee estas credenciales de variables de entorno. Contra
local hace falta tener `bun run dev` levantado. Todos los comandos aceptan
`--json`; úsalo siempre, la salida de texto es para personas.

```text
bun run series -- --url <url> --token <api-token> status --json
bun run series -- --url <url> --token <api-token> list [--status <estado>] --json
bun run series -- --url <url> --token <api-token> search "<texto>" --json
bun run series -- --url <url> --token <api-token> add --tmdb <tv:id|movie:id> --json
bun run series -- --url <url> --token <api-token> advance --entry <id> --json
bun run series -- --url <url> --token <api-token> transition --entry <id> --to <estado> --json
bun run series -- --url <url> --token <api-token> edit-entry --entry <id> [--locations <csv>] [--platforms <csv>] --json
bun run series -- --url <url> --token <api-token> abandon --entry <id> --reason "..." --json
bun run series -- --url <url> --token <api-token> discard --work <id> --reason "..." --json
bun run series -- --url <url> --token <api-token> sync-all [--no-wait] --json
```

Existe un posible atajo local `./series`, ignorado por Git, que fija URL y token
para los modos `local` y `prod`: `./series prod status --json`. Si no existe,
créalo siguiendo la plantilla de «Atajo local `./series`» del README. Nunca
versiones ni muestres sus tokens.
