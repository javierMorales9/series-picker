...
export type { Entry, EntryStatus, Work, WorkAggregate, WorkStatus };
...
export class SeriesApplication {
  ...
  getDashboard() {
    ...
    return {
      all,
      watching: all.filter((work) => work.status === "watching"),
      selected: all.filter((work) => work.status === "selected"),
      started: all.filter((work) => work.status === "started"),
